...
export default function AdminStudentsList() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setStudents(prev => prev.filter(u => u.id !== deleteTargetId))
    } catch (err: any) {
      console.error(err)
    ...
    <div className="max-w-7xl mx-auto px-6 md:px-12 pt-10 pb-12">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div className="space-y-2">
              ...
              <p className="text-text-secondary text-sm md:text-base">
                View, create, update, and delete student accounts.
              </p>
            </div>
            <Link
      ...
      <main className="animate-fade-in-up animate-delay-100">
        ...
        {!loading && !error && students.length === 0 && (
          <p className="text-center py-12 text-sm text-text-secondary">No students found.</p>
        )}
