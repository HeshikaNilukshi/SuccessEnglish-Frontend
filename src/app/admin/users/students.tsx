...
export default function AdminStudentsList() {
  ...
  const pageTitle = (
    <>
      Manage <span className="gradient-text-accent">Students</span>
    </>
  )
  ...
  return (
    <PageShell
      title={pageTitle}
      subtitle="View, create, update, and delete student accounts."
      ...
      <div className="flex-grow flex flex-col animate-fade-in-up">
        ...
        {!loading && !error && students.length === 0 && (
          <EmptyState
            ...
            description="There are currently no students registered."
          />
        )}
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/[0.04]">
                  {students.map(student => (
                    ...
                    >
                      <td className="p-5 text-sm font-semibold text-text-primary font-mono">{student.id}</td>
                      <td className="p-5 text-sm font-bold text-text-primary">{student.name}</td>
                      <td className="p-5 text-sm text-text-secondary">{student.email}</td>
                      <td className="p-5 text-sm text-text-secondary">{formatDate(student.createdAt!)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
